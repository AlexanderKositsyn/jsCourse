        ...
        result
          .then(towns => {
            ...
            towns.forEach((town, i, towns) => {
              assert.isTrue(
                town.hasOwnProperty("name"),
                "город должен иметь свойтво name"
              );
              // 'a'<'b' >>> true
              // towns[i - 1].name должно быть меньше чем town.name (т е)
              if (i) {
                assert.isBelow(
                  towns[i - 1].name,
                  town.name,
                  "города должны быть отсортированы"
                );
              }
            });
            done();
  ...
  describe("Интеграционное тестирование", () => {
    it("на старнице должны быть элементы с нужными id", () => {
      ...
      assert.instanceOf(
        loadingBlock,
        Element,
        "id элемента должен быть loading-block"
      );
      assert.isNotNull(filterBlock, "элемент не найден");
      assert.instanceOf(
        filterBlock,
        Element,
        "id элемента должен быть filter-block"
      );
      assert.isNotNull(filterInput, "элемент не найден");
      assert.instanceOf(
        filterInput,
        Element,
        "id элемента должен быть filter-input"
      );
      assert.isNotNull(filterResult, "элемент не найден");
      assert.instanceOf(
        filterResult,
        Element,
        "id элемента должен быть filter-result"
      );
    });
